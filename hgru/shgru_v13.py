...
class SHgruV13(nn.Module):
    def __init__(
        ...
        c1 = (embed_dim // expand_ratio) ** -0.5
        proj_weight = torch.empty(expand_ratio, embed_dim // expand_ratio, 3 * embed_dim).uniform_(-c1, c1)
    ...
    def forward(self, x, lower_bound=0):
        ...
        # out proj
        output_state = rearrange(output_state, 'n b (k d) -> n b k d', k=self.expand_ratio).sum(dim=-2)
