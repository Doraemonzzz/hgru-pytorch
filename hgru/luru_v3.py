...
class LuruV3(nn.Module):
    def __init__(
        ...
        self.input_proj = nn.Linear(embed_dim, 3 * embed_dim, bias=bias)
        
        theta = nn.Parameter(
            10000 ** (-2 / embed_dim * torch.arange(embed_dim // 2)).reshape(1, 1, -1),
            requires_grad=False,
        )
    ...
    def forward(self, x, **kwargs):
        # x: n, b, d
        qkv = self.act(self.input_proj(x))
        q, k, v = qkv.chunk(3, dim=-1)
        gate = F.sigmoid(self.gate(x))
        kv = k * v
        input_state = self.rotate(kv)
        output_state = self.scan(input_state)

        output_state = self.norm(gate * q * output_state)
