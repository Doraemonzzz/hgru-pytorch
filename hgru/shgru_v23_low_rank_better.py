...
class SHgruV23(nn.Module):
    def __init__(
        ...
        self.in_low_rank = nn.Sequential(
            nn.Linear(embed_dim, gate_dim, bias=bias),
            nn.Linear(gate_dim, expand_ratio * embed_dim, bias=bias),
        )
        self.forget_low_rank = nn.Sequential(
            nn.Linear(embed_dim, gate_dim, bias=bias),
            nn.Linear(gate_dim, expand_ratio * embed_dim, bias=bias),
        )
        self.down_low_rank = nn.Sequential(
            nn.Linear(expand_ratio * embed_dim, gate_dim, bias=bias),
    ...
    def forward(self, x, lower_bound=0):
        ...
        input = self.in_low_rank(self.act(input))
        forget_gate = self.forget_low_rank(self.act(output_gate))
        ...
        # down
        output_state = self.down_low_rank(output_state)
