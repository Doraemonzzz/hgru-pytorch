...
class SHgruV25(nn.Module):
    ...
    def forward(self, x, lower_bound=0):
        ...
        # up
        input, input_up_proj, forget_gate, forget_up_proj, down_proj = map(
            lambda x: rearrange(x, "... (k d) -> ... k d", k=self.expand_ratio),
            [input, self.input_up_proj, forget_gate, self.forget_up_proj, self.down_proj],
        )
        # k d, m d -> k d m
        input = torch.einsum('... k d, g d -> ... k d g', input, input_up_proj)
        forget_gate = torch.einsum('... k d, g d -> ... k d g', forget_gate, forget_up_proj)
        ...
        # mix
        lambda_ = torch.exp(lower_bound * F.sigmoid(forget_gate))
        ...
        output_state = torch.einsum('... k d g, g d -> ... k d', output_state, down_proj)
