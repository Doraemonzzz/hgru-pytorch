...
class SHgruV6(nn.Module):
    def __init__(
        ...
        # d -> k, d / k, -> k, d
        proj_weight = torch.randn(expand_ratio, embed_dim // expand_ratio, 3 * embed_dim) * 0.1
        self.proj_weight = nn.Parameter(
            proj_weight, requires_grad=True,
        )
        out_proj = torch.randn(expand_ratio, embed_dim, embed_dim // expand_ratio) * 0.1
        # k, d -> k, d / k
