...
class SHgruV52_2d_triton(nn.Module):
    def __init__(
        self,
        embed_dim,
        expand_ratio=2,
    ...
    def forward(self, x, lower_bound=0):
        ...
        # h
        V_h, Q_h, log_lambda_h, K_h = map(
            lambda x: rearrange(x, "H W b h d -> H (W b) h d"),
            [V, Q, log_lambda_, K],
        )
        output_state_h_forward = self.scan(V_h, Q_h, log_lambda_h, K_h)
        output_state_h_reverse = self.reverse_scan(V_h, Q_h, log_lambda_h, K_h)
        output_state_h_forward, output_state_h_reverse = map(
            lambda x: rearrange(x, "H (W b) h d -> H W b h d", W=W),
            [output_state_h_forward, output_state_h_reverse],
        )
        # w
        V_w, Q_w, log_lambda_w, K_w = map(
            lambda x: rearrange(x, "H W b h d -> W (H b) h d"),
            [V, Q, log_lambda_, K],
        )
        output_state_w_forward = self.scan(V_w, Q_w, log_lambda_w, K_w)
        output_state_w_reverse = self.reverse_scan(V_w, Q_w, log_lambda_w, K_w)
        output_state_w_forward, output_state_w_reverse = map(
            lambda x: rearrange(x, "W (H b) h d -> H W b h d", H=H),
            [output_state_w_forward, output_state_w_reverse],
        )
        
        # fusion
        output_state = output_state_h_forward + output_state_h_reverse \
                     + output_state_w_forward + output_state_w_reverse
