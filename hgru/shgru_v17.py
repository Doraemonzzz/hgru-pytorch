...
from .kr import KRLinear

class SHgruV17(nn.Module):
    def __init__(
        self,
        embed_dim,
        gate_dim=128,
        ...
        self.input_kr = KRLinear(embed_dim, expand_ratio)
        self.output_kr = KRLinear(embed_dim, expand_ratio)
        self.forget_kr = KRLinear(embed_dim, expand_ratio)
    ...
    def forward(self, x, lower_bound=0):
        ...
        input = self.input_kr(input)
        output_gate = self.output_kr(output_gate)
        forget_gate = self.forget_kr(forget_gate)
        ...
        # kd -> d, k
        output_state, output_gate = map(
            lambda x: rearrange(x, "b n (d k) -> b n d k", k=self.expand_ratio), [output_state, output_gate]
        )
        output_state = torch.einsum("... d, ... d -> ...", output_state, output_gate)
